...
def dtwDistance(s,t):
    ...
    for i in range(1,n):
        for j in range(1,m):
            dtw[i][j] = mdist(s[i-1],t[j-1])

    midTime = timeit.default_timer()
    for i in range(1,n):
        for j in range(1,m):
            dtw[i][j] = dtw[i][j] + min(dtw[i-1][j], dtw[i][j-1], dtw[i-1][j-1])
...
def KnnClassifier(X, y, K=10):
    ...
    for step, (train_index, test_index) in enumerate(kf.split(X, y)):
        ...
        classifier = KNeighborsClassifier(n_neighbors = 1, metric = dtwDistance)
...
def main():
    # Import dataset
    dataset_Test = pd.read_csv('C_ItalyPowerDemand.txt')
    X = dataset_Test.iloc[:,1:].values
    y = dataset_Test.iloc[:,0].values
    #print ("10% done")

    confusionMatrices, classifierReports, accuracies, runtimes  = KnnClassifier(X, y, 10)
    tCCT = 0
    tPCT = 0
    rT = 0
    print ("100% done")
